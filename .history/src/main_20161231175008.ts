...
let responseStream = requestStream.flatMap(requestUrl =>
    Observable.fromPromise(jQuery.jQuery.getJSON(requestUrl))
);
...
suggestionStream.suscribe(user =>
    console.log(user)
);

suggestionTwoStream.suscribe(user =>
    console.log(user)
);

suggestionThreeStream.suscribe(user =>
    console.log(user)
);
